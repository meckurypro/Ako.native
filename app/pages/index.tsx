...
function ModeRow({ name, subtitle, avatar, active, colors, onPress }: { name: string; subtitle: string; avatar: string | null | undefined; active: boolean; colors: any; onPress: () => void }) {
  return <Pressable onPress={onPress} style={[s.row, { borderBottomColor: colors.border }]}><Avatar uri={avatar} name={name} size={40} /><View style={s.rowCopy}><Text style={s.rowName}>{name}</Text><Text variant="caption" color="muted">{subtitle}</Text></View>{active ? <Icon name="check" size={20} color={colors.accent} /> : null}</Pressable>;
}
