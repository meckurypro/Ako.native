...
export function LikeHeart({ active, color }: { active: boolean; color: string }) {
  ...
  return <View style={s.root}><Animated.View style={heart}><Icon name="heart" size={24} color={color} fill={active ? color : "none"} /></Animated.View>{active && SPARKS.map(([x, y], i) => <Spark key={i} x={x} y={y} progress={burst} color={color} />)}</View>;
}
