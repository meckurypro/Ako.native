...
//
// Port of web's src/hooks/useOnboarding.ts — same queries, same RPC, same
// diff-based interest save. Only difference: useCompleteOnboarding also
// writes the new status straight into the cache (see there).
...
/**
 * Drives onboarding redirects (index.tsx, OnboardingGate,
 * OnboardingStepGuard) and resume behavior. `hasInterests` lets us skip
 * re-showing Welcome/Interests to someone who started onboarding, left,
 * and came back — without a separate "current step" column (see
 * sql/31_onboarding_recommendations.sql on web).
 */
...
export function useCompleteOnboarding() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      // Native addition: OnboardingGate lives in the root layout and
      // watches this same query, so leaving the cached `completed: false`
      // in place until the refetch lands could bounce the user straight
      // back into onboarding right after BuildingAko navigates to /feed.
      // Write the known-true value first, then still refetch to confirm.
      queryClient.setQueryData<OnboardingStatus>(["onboarding-status", user?.id], (prev) => ({
        hasInterests: prev?.hasInterests ?? true,
        completed: true,
      }));
      queryClient.invalidateQueries({ queryKey: ["onboarding-status", user?.id] });
    },
