...
//
// Direct port of web's src/hooks/useBookmarks.ts — plain supabase-js +
// react-query, no platform differences. The one omission: web gates each
// per-card query on DEBUG_DISABLE_PER_CARD_QUERIES (lib/debugFlags.ts, a
// temporary swipe-bounce diagnostic that's off by default), which isn't
// ported.
